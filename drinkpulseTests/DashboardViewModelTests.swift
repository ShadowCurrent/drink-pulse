...
struct DashboardViewModelTests {
    ...
    @Test func todayRiskLevel_safe_at49pct_whoMale() throws {
        ...
        let vm = DashboardViewModel()
        vm.profile = profile
        vm.events = [event(daysAgo: 0, grams: 9.8, in: c.mainContext)] // 49% of 20 g
        vm.now = .now
    ...
    @Test func todayRiskLevel_caution_at75pct_whoMale() throws {
        ...
        let vm = DashboardViewModel()
        vm.profile = profile
        vm.events = [event(daysAgo: 0, grams: 15, in: c.mainContext)] // 75% of 20 g
        vm.now = .now
    ...
    @Test func todayRiskLevel_exceeded_atOrOverDailyLimit_whoMale() throws {
        ...
        let vm = DashboardViewModel()
        vm.profile = profile
        vm.events = [event(daysAgo: 0, grams: 20, in: c.mainContext)] // exactly 100%
        vm.now = .now
