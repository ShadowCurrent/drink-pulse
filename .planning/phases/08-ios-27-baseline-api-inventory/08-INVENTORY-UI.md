...
| --- | --- | --- | --- |
| App entry / startup | `drinkpulse/drinkpulseApp.swift` | `App`, `WindowGroup`, `@State`, `@AppStorage`, `task`, model-container startup, notification delegation, color scheme | **NC-UI-01 — no-candidate:** aside from UI-C-07’s current Observation ownership check, no official deprecation or iOS 27-specific replacement lead was found. Startup/lifecycle semantics remain unchanged. |
