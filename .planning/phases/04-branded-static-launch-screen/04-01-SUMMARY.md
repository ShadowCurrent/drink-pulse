...
**LaunchIcon/LaunchBackground Asset Catalog entries wired through a standalone `drinkpulse/Info.plist`'s `UILaunchScreen` dict (Pattern 2 fallback), replacing the auto-generated blank launch screen -- Tasks 1-2 complete and committed; Task 3's real-device cold-launch verification is a blocking checkpoint awaiting human execution (round 6: structural build-config investigation found no code defect -- an actual iphoneos-SDK build byte-matches every prior simulator check, so the next step is a human-only diagnostic: pulling the actually-installed app container off the device via Xcode's Devices window).**
...
**BLOCKED at Task 3 (round 6+).** Round 6's report (pixel-identical symptom across two structurally different `LaunchIcon` pixel treatments -- alpha-masked in round 2/`072c811` vs. fully-opaque baked backgrounds in round 5/`72ea4ff` -- with the correct project path and a full physical device reboot both independently confirmed by the human) rules out image content, stale cache, and wrong-checkout/branch as explanations. Round 6's own investigation (this SUMMARY's Deviation 8) then structurally ruled out every remaining cause reachable from this environment: no legacy `LaunchScreen.storyboard`/`UILaunchStoryboardName` exists anywhere in the repo; no SDK-conditional (`[sdk=iphoneos*]`) build-setting override exists on any relevant key; only one scheme/one app target exists; and -- for the first time across all six rounds -- an actual `iphoneos`-SDK build (`xcodebuild -destination 'generic/platform=iOS' ... CODE_SIGNING_ALLOWED=NO`) was produced and its compiled `Info.plist`/`Assets.car` inspected directly, and found byte-structurally identical to what every prior simulator-based check already verified correct.

**No root cause was found within this environment's inspection capability.** This is reported honestly rather than fabricated -- every reachable structural angle came back clean. The remaining gap is a real Xcode "Run" to a physical device, which involves code-signing and provisioning-profile embedding + on-device installation that a CLI-only, unsigned build (no device attached, no signing team configured in this environment) cannot replicate or inspect.

**Recommended next diagnostic (has not been done by any round yet):** on the human's own machine, connect the physical device to Xcode, open **Window > Devices and Simulators**, select the device, find `drinkpulse` under Installed Apps, and use the gear icon's **"Download Container..."** to pull the *actual, currently-installed* app bundle directly off the device. Then inspect that container's `Info.plist` (`plutil -p`) and `Assets.car` (`assetutil --info`, or `xcrun --sdk iphoneos assetutil --info`) exactly as this SUMMARY's rounds have been doing for CLI-built artifacts. If the on-device container's `Info.plist` lacks `UILaunchScreen`, or its `Assets.car` lacks the `LaunchIcon`/`LaunchBackground` renditions, or (unlikely but possible) the container's build isn't actually at `72ea4ff` or later -- that is definitive proof of what's actually installed on the device, something no CLI build from this environment can ever confirm. This closes the one remaining gap between "what we can prove is correct in the repository and in every build product we can produce" and "what is genuinely running on the physical hardware."

Before this plan (and Phase 4) can be considered complete:

1. The human performs the "Download Container" diagnostic above and reports what the on-device `Info.plist`/`Assets.car` actually contain.
2. If that container matches what this SUMMARY has verified (correct `UILaunchScreen` wiring, correct `LaunchIcon`/`LaunchBackground` renditions) -- the divergence is happening at Springboard's rendering step itself, which would need to be investigated on the human's own machine with Console.app device logs during a cold launch (out of this environment's reach entirely).
3. If that container does NOT match (missing keys, wrong build) -- the actual root cause is a signing/deployment issue (stale provisioning profile, wrong build configuration selected in Xcode's scheme, or similar), not anything in this repository's source.
4. Once a genuine root cause is identified via the above, fix it, rebuild, and re-verify via a fresh Task 3 checkpoint round (force-quit cold launch, light + dark mode, per this plan's original `how-to-verify` steps).
5. Only after explicit approval of Task 3's checkpoint should `LAUNCH-01` be marked complete in `REQUIREMENTS.md` and this plan's status updated to `complete`.
