...
**BLOCKED at Task 3 (round 4).** All work, including the round-2 `LaunchIcon.png` alpha-mask fix (`072c811`), is verified present and correct on `main` at the primary checkout via a from-scratch clean rebuild (see Deviation 6 above) -- no further code change was made this round. Before this plan (and Phase 4) can be considered complete:

1. A human must do a genuinely clean rebuild-and-reinstall cycle (not a plain reinstall over the existing app) on a real physical iOS device from the primary checkout (`/Users/fempter/Developer/drinkpulse`, `main` branch): in Xcode, Product -> Clean Build Folder (Cmd+Shift+K); fully delete the app from the device (long-press the icon -> Remove App -> Delete App -- not just force-quit); optionally also delete `~/Library/Developer/Xcode/DerivedData/drinkpulse-*` if Clean Build Folder alone doesn't clear it; then rebuild and install fresh.
2. Fully force-quit the freshly installed app, cold-launch it, and confirm: branded icon **rounded** (matching the Home Screen icon's squircle shape, not a hard-edged square) and centered at Home-Screen-icon size on a plain white (light) / black (dark) background, no text/spinner/animation, no visible flash or halo at handoff into the first live frame -- in both light and dark mode.
3. If the same symptom still appears after a genuinely clean rebuild + full device delete, that is strong evidence the round-2 fix is somehow not reaching the device through a channel this investigation couldn't inspect (e.g. a provisioning/signing artifact, or an even-deeper iOS render cache) -- report back explicitly that a full clean rebuild + device delete was tried and the issue persisted, so the next round doesn't have to re-derive this and can jump straight to a different investigation angle.
