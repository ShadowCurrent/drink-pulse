...
**BLOCKED at Task 3 (round 5).** Round 4 ruled out stale cache as the explanation for the recurring sharp-corner symptom (the human did a genuine Clean Build Folder + full device app deletion and still saw the identical issue). Rather than re-verify the same alpha fix a third time, this round changed strategy: `LaunchIcon` no longer relies on PNG alpha transparency at all -- the rounded shape is now baked into two fully-opaque PNGs (`LaunchIcon.png` white corners, `LaunchIcon-dark.png` black corners) via a dual-appearance `Contents.json`, committed in `72ea4ff` (see Deviation 7). This guarantees correct visual results regardless of how the platform's launch-screen compositor handles alpha, because there is no transparency left to mishandle.

**Working hypothesis, not confirmed fact:** the likely root cause is that `UILaunchScreen`'s rendering path (SpringBoard/backboardd, pre-UIKit) does not honor PNG alpha the same way normal UIKit compositing does -- this is inferred from process of elimination (the alpha fix verified correct at every inspectable layer, twice, yet failed on real hardware even after a full cache-bust), not from any documented Apple source or direct inspection of SpringBoard internals.

Before this plan (and Phase 4) can be considered complete:

1. A human must build and install the app onto a real physical iOS device from the primary checkout (`/Users/fempter/Developer/drinkpulse`, `main` branch, commit `72ea4ff` or later).
2. Fully force-quit the app (App Switcher, swipe away), then cold-launch by tapping the Home Screen icon. Confirm: branded icon **rounded** (matching the Home Screen icon's squircle shape, not a hard-edged square), centered at Home-Screen-icon size, corners blending seamlessly into a plain white background, no text/spinner/animation.
3. Watch the handoff into the first live frame (Onboarding Welcome or Dashboard) -- confirm no visible color flash or mismatch.
4. Switch to Dark Mode in Settings > Display & Brightness, repeat steps 2-3 -- confirm corners blend seamlessly into a plain black background.
5. If the icon looks noticeably larger, differently cropped, or off-center, or a flash/halo is visible at handoff, describe the specific issue instead of approving.
6. **If this round-5 approach also shows the same sharp-corner/color-bleed symptom:** that would be very strong evidence the problem is unrelated to corner/alpha treatment entirely -- since there is now no alpha channel in the shipped asset for the platform to mishandle. The next investigation angle in that case should shift to how the image is *referenced* rather than how it's drawn: re-verify the exact `UIImageName` string match against the Asset Catalog name (case-sensitivity, hidden whitespace), check whether the `2x`-only scale slot is being mis-resolved on the specific test device's actual scale factor, or consider whether the device is somehow still launching a different (non-`main`) build. Report explicitly that round 5's solid-background approach was tried and still failed, so a future round can jump straight to this angle instead of re-deriving it.
7. Only after explicit approval of Task 3's checkpoint should `LAUNCH-01` be marked complete in `REQUIREMENTS.md` and this plan's status updated to `complete`.
