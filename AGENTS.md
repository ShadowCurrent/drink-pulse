...
## Apple-platform source-of-truth policy

For every Swift, SwiftUI, Xcode, XCTest/XCUITest, iOS SDK, Simulator, accessibility, or other Apple-framework decision:

1. Consult official Apple documentation first, using Xcode MCP `DocumentationSearch` whenever it is available; use `developer.apple.com` only if Xcode MCP cannot provide the needed documentation.
2. Use Xcode MCP for workspace reads, diagnostics, builds, tests, and device/simulator interaction whenever it is available.
3. Use compiler diagnostics to verify an implementation, never as a substitute for official API documentation.
4. Skills, plugins, and third-party sources may add implementation context, but must not replace Apple documentation or independently determine an API migration/replacement decision.
5. Record the official source, API availability, and rationale for every Apple API change.

Never infer that an API is deprecated, obsolete, or replaceable merely because it is old.
...
## No guessing: verify against official Apple docs before implementing

Never guess how to implement an unfamiliar API, framework behavior, or
platform mechanic. Before implementing, consult official Apple documentation
through Xcode MCP `DocumentationSearch`; use developer.apple.com only when
MCP documentation is unavailable. Check the exact iOS version this project
...
case, reputable third-party iOS developer resources may supplement the
research — but Apple's own documentation remains the primary source of truth
and Xcode MCP remains the preferred project/API interface.
...
## Use skills only as supplementary implementation guidance

Before planning or researching a feature — GSD workflow or ad-hoc — identify
relevant installed skills/plugins only after consulting Apple documentation
and Xcode MCP. For example, `swiftui-expert-skill` may help with SwiftUI view
design, state flow, list/ForEach identity, performance, Liquid Glass, or
Instruments, but it supplements rather than replaces Apple documentation.
This applies to planning and research agents too: cite the official source
actually consulted, not a skill's secondary summary.
